...
if (BATCHER === PIXI.WebGLRenderer.BATCH_GROUPS) {
	console.log("Group Batching");
	console.log("Dirtying Sprites?", DIRTY_SPRITES);
} else {
	console.log("Single-Buffer Batching: "+ 
		((BATCHER===PIXI.WebGLRenderer.BATCH_SIMPLE) ? "Simple":"Multitexture"));
	console.log("Batch Size:", PIXI.WebGLRenderer.batchSize);
}
console.log("Blending enabled?", PIXI.blendingEnabled);

var step = 0;
function texIndex(i) {
	switch (TEXTURE_BENCH) {
		case 'worst':
			return i % textures.length;
		case 'best':
			if (i % textures.length == 0)
				step++;
			return step % textures.length;
		case 'random':
			return ~~(Math.random() * textures.length);	
		//number
		default:
			return i % Math.min(textures.length, TEXTURE_BENCH);
	}
}

function onReady()
{
	PIXI.WebGLRenderer.batchMode = BATCHER;
	...
	var urls = ["spinObj_01.png",
					 "spinObj_02.png",
					"spinObj_03.png", "spinObj_04.png",
					"spinObj_05.png", "spinObj_06.png",
					"spinObj_07.png", "spinObj_08.png"

					// "bunny.png", "button.png", "panda.png",
					// "logo_small.png", "click.png"
				];
	for (var i=0; i<urls.length; i++) {
		textures.push( new PIXI.Texture.fromImage(urls[i]) );
	}
	...
	{
		//this is "worst case" scenario with texture packing
		var tex = textures[ texIndex(i) ];
		var bunny = new PIXI.Sprite(tex, {x:0, y:0, width:26, height:37});
...
function update()
{
	...
	for (var i = 0; i < bunnys.length; i++) 
	{
		...
		// update cached info every frame...
		if (DIRTY_SPRITES) {
			bunny.alpha = Math.random();
			bunny.updateFrame = true;
		}
